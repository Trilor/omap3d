...
## アーキテクチャ維持ルール（全開発に恒久適用）

### 肥大化の禁止
- `app.js` や単一ファイルに処理を集中させない
- 機能（Feature）やドメインごとにディレクトリを分割する
...
- 他モジュールの内部状態を直接読み取って分岐しない（呼び出し側は「命じる」だけ）
- 直接 import が必要な場合は、依存方向を必ず `core → api`、`features → core` の一方向に保つ
- UI コンポーネントからドメインロジックを直接呼び出すのは禁止（eventBus 経由）

### 単一責任の原則（SRP）
- 1ファイル = 1目的。「何でも」「汎用」な名前（Common, Util, Manager）は避ける
